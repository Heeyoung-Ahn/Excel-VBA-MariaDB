-- 담당자별 예결산 금액을 특정화폐로 집계하는 쿼리 샘플

-- 1-1) 집계합수에 서브쿼리로 환율을 반환하여 달러화 화폐로 계산
...
-- 1-2) 집계합수에 서브쿼리로 환율을 반환하여 원화 화폐로 계산
SELECT 	a.bs_gb, a.user_nm, COUNT(a.bs_gb), SUM(a.bs_amt_krw),
			ROUND(SUM(a.bs_amt * 	(CASE a.currency_un
												WHEN 'KRW' THEN 1
												ELSE (SELECT b.fx_rate_krw FROM co_account.currencies b WHERE b.currency_un = a.currency_un)
											END)), 2) AS bs_amt_usd
 FROM v_budget_settlement a
 WHERE a.suspended = 0
 GROUP BY a.bs_gb, a.user_nm WITH ROLLUP;
 
-- 1-3) 집계합수에 서브쿼리로 환율을 반환하여 유로화 화폐로 계산
SELECT 	a.bs_gb, a.user_nm, COUNT(a.bs_gb), SUM(a.bs_amt_krw),
			ROUND(SUM(a.bs_amt * 	(CASE a.currency_un
												WHEN 'EUR' THEN 1
												ELSE (SELECT b.fx_rate_usd FROM co_account.currencies b WHERE b.currency_un = a.currency_un) /
...
-- 2) 공통테이블표현식으로 달러화 환율을 적용한 임시 테이블 생성 후 집계
